...
export async function POST(
  ...
  try {
    ...
    console.info("[campaign launch] inngest.send result", {
      campaignId,
    ...
    console.error("[campaign launch] inngest.send failed", { campaignId, error: message });
    await supabase.from("campaigns").update({ status: "draft" }).eq("id", campaignId);
    return NextResponse.json(
      { error: "Failed to launch campaign. Please retry." },
      { status: 502 },
    );
  }
}
